#!/usr/bin/env python3
"""
Step 2 (Autonomous): AlphaGenome AVI Screening for 93,929 Autonomous Microproteins.

Queries dense AlphaGenome Variant Impact (AVI) purifying selection scores across
genuine autonomous microprotein intervals (non-intORFs).

Features:
- Prioritizes high-confidence structured microproteins (sorted by AlphaFold pLDDT descending).
- Rate-balanced worker concurrency (default: 3 workers) with adaptive pacing to prevent quota exhaustion.
- Checkpointed Parquet batches (default: 200 ORFs/batch).
- Automatic resumption skipping previously scored ORFs.
- Real-time JSON telemetry monitor (`hmpa_autonomous_avi_status.json`).
"""
...
AUTONOMOUS_FILE = DATA_DIR / "curated" / "hmpa_autonomous_intervals.parquet"
...
def query_single_microprotein(client, row: dict) -> dict:
    """
    Query dense AVI scores across a single autonomous microprotein interval.
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "RESOURCE_EXHAUSTED" in err_str or "Quota exceeded" in err_str:
                ...
                tqdm.write(f"[{smorf_id}] Quota throttle. Pausing {wait_time}s before retry ({attempt+1}/{max_retries})...")
    ...
    # Slight inter-query pacing to prevent API burst spikes
    time.sleep(0.15)
    ...
    return {
        ...
        "cds_host_genes": str(row.get("cds_host_genes", "")),
...
def main():
    parser = argparse.ArgumentParser(description="HMPA Autonomous Microproteins AlphaGenome AVI Screener.")
    parser.add_argument("--workers", type=int, default=3, help="Concurrent worker threads (default: 3 to avoid quota limits)")
    parser.add_argument("--batch-size", type=int, default=200, help="Checkpointed batch size (default: 200)")
    parser.add_argument("--max-orfs", type=int, default=None, help="Stop after scoring N orfs (optional)")
    parser.add_argument("--sort-plddt", action="store_true", default=True, help="Sort by AlphaFold pLDDT descending")
    ...
    print("=" * 80)
    print("HMPA Autonomous Microprotein Screening Daemon (Non-intORFs)")
    print(f"Workers: {args.workers} | Batch Size: {args.batch_size} | Max ORFs: {args.max_orfs or 'All Autonomous'}")
    print("=" * 80)

    print(f"Loading autonomous intervals: {AUTONOMOUS_FILE}...")
    df_catalog = pd.read_parquet(AUTONOMOUS_FILE)
    total_in_file = len(df_catalog)
    print(f"Loaded {total_in_file:,} autonomous microprotein intervals.")

    # Sort by pLDDT descending so top structured candidates get scored first
    if args.sort_plddt:
        df_catalog = df_catalog.sort_values("plddt", ascending=False).reset_index(drop=True)
        print("Prioritized candidates by AlphaFold pLDDT (highest confidence first).")
    ...
    print(f"Remaining autonomous candidates to score: {len(uncompleted_df):,}")

    if len(uncompleted_df) == 0:
        print("All autonomous candidates are already completed!")
        return
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        pbar = tqdm(total=len(rows_to_process), desc="Scoring Autonomous smORFs", unit="orf")
    ...
    print(f"\nDone! Session finished in {elapsed:.1f}s. Total completed: {total_completed:,} / {total_in_file:,}.")
