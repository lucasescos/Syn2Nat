#!/usr/bin/env python3
"""
Self-Healing Supervisor Daemon for 617k HMPA Microprotein AlphaGenome Screening.
Continuously runs cycles of `hmpa_step2_query_alphagenome_avi.py` until all 617,262
microproteins are fully scored across Google DeepMind's AlphaGenome Atlas.

Features:
- Subprocess cycle management: refreshes memory & OS resources every N sequences.
- Auto-restart with exponential backoff upon transient network/API rate limits.
- Real-time progress telemetry (completed count, percentage, throughput, ETA).
...
def parse_args():
    parser = argparse.ArgumentParser(description="Supervisor Daemon for HMPA AlphaGenome Screening.")
    parser.add_argument("--chunk-size", type=int, default=500, help="Microproteins per cycle (default: 500)")
    parser.add_argument("--workers", type=int, default=6, help="Concurrent API workers per cycle (default: 6)")
    parser.add_argument("--batch-size", type=int, default=250, help="Parquet checkpoint size (default: 250)")
    parser.add_argument("--max-retries", type=int, default=50, help="Max consecutive failure retries")
...
def main():
    args = parse_args()
    print("=" * 80)
    print("[*] HMPA AlphaGenome Screening Supervisor Daemon")
    ...
    while True:
        ...
        if remaining <= 0:
            print("\n" + "=" * 80)
            print("[SUCCESS] ALL 617,262 MICROPROTEINS HAVE BEEN FULLY SCORED!")
            print("=" * 80)
            break
        ...
        cmd = [
            ...
            "python",
            str(ROOT_DIR / "scripts" / "hmpa_step2_query_alphagenome_avi.py"),
            "--max-orfs", str(chunk),
